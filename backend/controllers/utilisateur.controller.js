...
exports.connexion = async (req, res, next) => {
  let fetchedUser;
  let user = await Utilisateur.findOne({ nomUtilisateur: req.body.nomUtilisateur })
  // console.log(user)
    try {
      if (!user) {
      ...
      .then(result => {
        ...
        const token = jwt.sign(
          { utilisateurId: fetchedUser._id, nom: fetchedUser.nom, nomUtilisateur: fetchedUser.nomUtilisateur },
          "atelier",
          { expiresIn: "24h" }
        );
